...
// Upper bound on any single backend call. The backend is a separate origin
// (Render in production, a rewrite target in development), so a request can hang
// rather than fail fast when that origin is down.
...
function Metric({ label, value, unit = "" }: { label: string; value: unknown; unit?: string }) { const v = value === null || value === undefined ? "NO DATA" : String(value); return <div className="metric"><span>{label}</span><strong>{v}</strong>{unit && v !== "NO DATA" ? <em>{unit}</em> : null}</div>; }
function Status({ ok, text }: { ok: boolean; text?: string }) { return <span className={`status ${ok ? "ok" : "bad"}`}><i />{text ?? (ok ? "CONNECTED" : "UNAVAILABLE")}</span>; }
function val(obj: any, path: string[], fallback: any = null): any { return path.reduce((a, k) => a?.[k], obj) ?? fallback; }
function Json({ data }: { data: any }) { return <pre className="json">{data == null ? "NO DATA / SERVICE UNAVAILABLE" : JSON.stringify(data, null, 2)}</pre>; }
...
export default function ConsolePage() {
  ...
  const [layers, setLayers] = useState<LayerState>({ rainfall: true, temperature: false, lst: false, sst: false, anomalies: false, risk: false, events: false });
  const [zoom, setZoom] = useState<{ type: "in" | "out" | "reset"; nonce: number }>();
  const [climateLayerStatus, setClimateLayerStatus] = useState<any>(null);
  ...
  const [scenario, setScenario] = useState({ precip: -15, temp: 2, sea: 0.4 });
  ...
  // A hung backend (unreachable host, black-holed proxy) leaves fetch pending
  // indefinitely, which pinned the UI on "LOADING INDIA CLIMATE STATE…" forever.
  // Bound every request so the interface always reaches a truthful final state.
  const apiFetch = useCallback(async (key: string, url: string) => { try { const r = await fetch(url, { cache: "no-store", signal: AbortSignal.timeout(API_TIMEOUT_MS) }); return [key, r.ok ? await r.json() : null] as const; } catch { return [key, null] as const; } }, []);
  ...
  const chooseState = useCallback((name: string) => {
    ...
    const found = id ? { id, name } : data.hierarchy?.states_and_union_territories?.find((s: any) => s.name.toLowerCase() === normalized);
    if (found) { setSelectedId(found.id); setSelectedName((found.name || name).toUpperCase()); setNav("Overview"); }
    else if (normalized) setModal("location");
  }, [data.hierarchy]);
  ...
  // A layer toggle must never imply data exists. The backend contract in
  // /api/climate/layers is the single source of truth for availability, and any
  // layer that is not "connected" is surfaced as unavailable rather than
  // silently rendering nothing.
  const layerContract: Record<string, any> = data.layerStatus?.layers ?? {};
  const layerState = (key: string) => String(layerContract?.[key]?.status ?? "").toLowerCase();
  const layerUnavailable = (key: string) => { const s = layerState(key); return s !== "" && s !== "connected"; };
  const LAYER_KEYS = ["rainfall", "temperature", "lst", "sst", "anomalies", "risk", "events"] as const;
  const layerLabel = (key: string) => key === "lst" ? "LAND SURFACE TEMP" : key === "sst" ? "SEA SURFACE TEMP" : key.toUpperCase();
  const unavailableOn = LAYER_KEYS.filter(k => layers[k] && layerUnavailable(k));
  ...
  const map = <div className="map-wrap"><GodsEyeOperations date={date} onDateChange={setDate}/><div className="godseye-hud"><span>GOD&#39;S-EYE / INDIA</span><b>CLIMATE INTELLIGENCE</b><small>{date} · PROVIDER-BACKED OBSERVATIONS</small></div><ClimateMap layers={layers} date={date} selectedState={selectedName} districtMetrics={districtMetricsById} onStateSelect={chooseState} onCoords={(lat, lon) => setCoords(`${lat.toFixed(4)}° N, ${lon.toFixed(4)}° E`)} zoomRequest={zoom}/><div className="map-actions"><button onClick={() => setZoom({ type: "in", nonce: Date.now() })}>+</button><button onClick={() => setZoom({ type: "out", nonce: Date.now() })}>−</button><button onClick={() => setZoom({ type: "reset", nonce: Date.now() })}>⌂</button></div><div className="map-date">OBSERVATION <b>{date}</b></div><div className="map-layers"><b>LAYERS</b>{LAYER_KEYS.map(k => { const off = layerUnavailable(k); return <button key={k} className={off ? "unavailable" : layers[k] ? "on" : ""} title={off ? `${layerContract?.[k]?.status} — no validated provider data` : `Toggle ${layerLabel(k)}`} aria-pressed={layers[k]} onClick={() => setLayers(x => ({ ...x, [k]: !x[k] }))}>{layerLabel(k)}<span className={layers[k] ? "switch on" : "switch"}/>{off && <em className="layer-flag">NO DATA</em>}</button>; })}</div>{unavailableOn.length > 0 && <div className="layer-warning" role="status">NO VALIDATED DATA FOR {unavailableOn.map(k => layerLabel(k)).join(", ")} — PROVIDER REQUIRED. The toggle is shown for transparency; nothing is estimated or drawn.</div>}<div className="map-coords">{coords}</div></div>;
  ...
  else if (nav === "Extreme Events") body = <div className="two"><Card title="EVENT MONITOR" action={<button className="outline" onClick={() => downloadEvents(events)}>EXPORT CSV</button>}><div className="event-kpis"><Metric label="TOTAL EXTREME POINTS" value={val(events,["summary","total_extreme_points"])} /><Metric label="HEAVY" value={val(events,["summary","heavy_points"])} /><Metric label="VERY HEAVY" value={val(events,["summary","very_heavy_points"])} /><Metric label="EXTREME" value={val(events,["summary","extremely_heavy_points"])} /></div><Json data={events}/></Card>{map}</div>;
  else if (nav === "What-If") body = <div className="two"><Card title="FORCING PARAMETERS"><Slider label="PRECIPITATION CHANGE" value={scenario.precip} min={-100} max={300} step={1} unit="%" onChange={v=>setScenario(s=>({...s,precip:v}))}/><Slider label="TEMPERATURE CHANGE" value={scenario.temp} min={-10} max={10} step={0.5} unit="°C" onChange={v=>setScenario(s=>({...s,temp:v}))}/><Slider label="SEA LEVEL RISE" value={scenario.sea} min={0} max={2} step={0.1} unit="m" onChange={v=>setScenario(s=>({...s,sea:v}))}/><button className="primary" disabled={simulating} onClick={runSimulation}>{simulating ? "RUNNING..." : "RUN SIMULATION"}</button><p className="notice">SENSITIVITY EXPERIMENT · not a full physical multi-hazard simulation.</p></Card><Card title="SCENARIO IMPACT"><Json data={simulation}/></Card></div>;
...
function Source({name,value}:{name:string;value:string}){return <div className="source"><span>{name}</span><Status ok={value!=="UNAVAILABLE"} text={value}/></div>}
