...
from backend.services.digital_twin_service import get_twin_state, get_historical_rainfall, get_baseline_forecast, get_model_catalog, get_validation_summary, explain_rainfall_risk, get_provenance, get_system_health
...
app = FastAPI(title="India Climate Digital Twin API", description="Operational scientific API for the India Climate Digital Twin.", version="0.9.3")
...
@app.get("/")
def root():
    return {"project": "India Climate Digital Twin", "status": "online", "engine": "Python + FastAPI + India Climate Twin Core", "version": "0.9.3"}
...
def climate_variable_state(variable: str, date: str | None = Query(default=None)):
    return {"state": _call(build_climate_state, date), "variable_discovery": discover_variable(variable)}
@app.get("/api/twin/multi-hazard")
def multi_hazard(date: str | None = Query(default=None)): return calculate_hazards(_call(build_climate_state, date))
@app.get("/api/twin/synchronize")
def twin_synchronize(date: str | None = Query(default=None)): return _call(synchronize, date)
...
def twin_what_if(base_date: str = Query(...), precipitation_delta_pct: float = Query(default=0.0, ge=-100, le=300), temperature_delta_c: float = Query(default=0.0, ge=-10, le=10), sea_level_rise_m: float = Query(default=0.0, ge=0, le=2), scenario: str = Query(default="custom", min_length=1, max_length=80)): return _call(build_what_if, base_date, precipitation_delta_pct, temperature_delta_c, sea_level_rise_m, scenario)
