...
from backend.services.extreme_event_service import detect_extreme_rainfall,get_extreme_rainfall_geojson,get_extreme_event_summary
from backend.services.climate_risk_service import get_climate_risk_summary,get_climate_risk_grid
from backend.services.digital_twin_service import get_twin_state_summary,get_twin_state,get_historical_rainfall,get_baseline_forecast,get_model_catalog,get_validation_summary,explain_rainfall_risk,simulate_rainfall_scenario,get_provenance,get_system_health
from backend.services.prithvi_wxc_service import get_prithvi_wxc_status,validate_prithvi_inputs,run_local_inference
from backend.services.prithvi_wxc_forecast_service import get_prithvi_forecast_status,generate_prithvi_forecast
app=FastAPI(title="India Climate Digital Twin API",description="Operational scientific API for the India Climate Digital Twin.",version="0.5.0")
...
@app.get("/")
def root():return {"project":"India Climate Digital Twin","status":"online","engine":"Python + FastAPI","version":"0.5.0"}
@app.get("/api/status")
...
def twin_summary():return _call(get_twin_state_summary)
...
def historical_rainfall(start:str|None=Query(default=None),end:str|None=Query(default=None),limit:int=Query(default=365,ge=1,le=5000)):return _call(get_historical_rainfall,start,end,limit)
...
@app.get("/api/forecast/prithvi-wxc/status")
def prithvi_forecast_status():return _call(get_prithvi_forecast_status)
@app.post("/api/forecast/prithvi-wxc")
def prithvi_forecast():return _call(generate_prithvi_forecast)
...
def scenario_simulate(base_date:str=Query(...),precipitation_delta_pct:float=Query(default=0.0,ge=-100,le=300),temperature_delta_c:float=Query(default=0.0,ge=-10,le=10),sea_level_rise_m:float=Query(default=0.0,ge=0,le=2),scenario:str=Query(default="custom",min_length=1,max_length=80)):return _call(simulate_rainfall_scenario,base_date,precipitation_delta_pct,temperature_delta_c,sea_level_rise_m,scenario)
